...
describe('Cadastro de Eventos', () => {
  it('Deve acessar a página de login, preencher os campos de email e senha corretamente e cadastrar um novo evento com sucesso', () => {
  ...
  it.only('Deve acessar a página inicial e cadastrar um novo evento com sucesso', () => {
    ...
    cy.visit('login.html');
    cy.get('#email').type('admin@ufu.br').should('have.value', 'admin@ufu.br'); // Valida email digitado
    cy.get('#password').type('tms-melhor-disciplina');
    cy.get('.btn-submit')
    ...
    cy.get('#admin-category')
      .select('Tecnologia')
      .should('have.value', 'tecnologia'); // Valida a seleção
    ...
    cy.get('input[name="admin-audience"][value="estudantes"]')
      .check()
      .should('be.checked'); // Valida se o radio foi marcado
    ...
    cy.get('#admin-description')
      ...
      .should('have.value', descriptionText); // Valida a descrição
    ...
    cy.get('#admin-location')
      .type(eventLocation)
      .should('have.value', eventLocation); // Valida o local
    ...
    cy.get('#admin-prereq-chips .admin-chip')
      .should('have.length', 2)
      .first()
      .should('contain.text', 'Mínimo 2º período'); // Valida o texto do primeiro chip
